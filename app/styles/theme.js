...
const hexToRgb = input => {
  ...
  if (!hexRegex.test(hex) || (hex.length !== 3 && hex.length !== 6)) {
  ...
  if (input.length === 3) {
    ...
    const last = input[2];
    hex = first + first + second + second + last + last;
  }
  hex = hex.toUpperCase(hex);
  const first = hex[0] + hex[1];
  const second = hex[2] + hex[3];
  const last = hex[4] + hex[5];
...
const sectionDark = {
  ...
  background: `radial-gradient(ellipse at center,${grayColor[4]} 0,${
    grayColor[5]
  } 100%)`,
};
